		...
		if(v.uid === 'dashboard'){
			...
			options  = `<div class="btn-remove" uid="${v.uid}" onclick="select(event)"></div>
				        	<div class="select select_${v.uid}">
				        		<div class="option" onclick="deleteSession(this)" uid="${v.uid}"><div class="icon-option" style="background-image: url(icones/delete.png);"></div>Excluir Sessão</div>
				        		<div class="option" onclick="edit(event)" uid="${v.uid}"><div class="icon-option" style="background-image: url(icones/edit.png);"></div>Renomar Sessão</div>
			...
			if(v.type === 'whatsapp'){
				iconetype = './icones/whatsapp2.png';
			}else{
				iconetype = '';
			}
